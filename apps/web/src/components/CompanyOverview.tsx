        ...
        <div className="bg-white rounded-lg shadow-xs p-4">
          ...
          <div className="divide-y divide-divider max-h-64 overflow-y-auto">
            ...
            {stats.overdueList.map((t) => (
              <div key={t.id} onClick={() => t.projectId && navigate(`/projects/${t.projectId}?task=${t.id}`)} className="flex items-center gap-2.5 py-2 cursor-pointer hover:bg-hover -mx-1 px-1 rounded-lg">
          ...
          <div className="divide-y divide-divider max-h-64 overflow-y-auto">
            ...
            {stats.dueSoonList.map((t) => (
              <div key={t.id} onClick={() => t.projectId && navigate(`/projects/${t.projectId}?task=${t.id}`)} className="flex items-center gap-2.5 py-2 cursor-pointer hover:bg-hover -mx-1 px-1 rounded-lg">
                ...
                <span className="text-[11px] text-dim shrink-0">{fmtThaiDate(t.dueDate)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-xs p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {stats.teamWorkload.map((u) => (
            <div key={u.id} className="flex items-center gap-3 p-2.5 rounded-lg bg-hover/60">
              <Avatar name={u.name} avatarUrl={u.avatarUrl} className="w-8 h-8 text-xs" colorClass={avatarColor(u.name)} />
              <div className="min-w-0 flex-1">
                ...
                <div className="h-1.5 bg-divider rounded-full overflow-hidden mt-1">
